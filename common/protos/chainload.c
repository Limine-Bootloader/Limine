...
noreturn void chainload(char *config, char *cmdline) {
    ...
    // Sanity check for native_load
    if (native_load) {
        // Check for compression marker ($)
        if (strchr(image_path, '$') != NULL) {
            panic(true, "efi: NATIVE_LOAD is incompatible with transparent compression ($). The firmware cannot read compressed bytes directly.");
        }
        
        // Check for hashing marker (#)
        if (strchr(image_path, '#') != NULL) {
            panic(true, "efi: NATIVE_LOAD is incompatible with URI hashing (#) due to TOCTOU vulnerabilities.");
        }
    }
